...
const {userRoleUser, userRoleModerator} = require("../fixtures/users");
...
describe("Grant Access Control", () => {
  ...
  describe("Guest", () => {
    ...
    it("Should can't read", async () => {
      const response = await request.get(rootUrl);
      assert.equal(response.statusCode, 401);
    });
    it("Should can't update", async () => {
      const response = await request.patch({
        url: `${rootUrl}/${grantList[0]._id}`,
        json: true,
        body: {
          attributes: ["*", "!_id", "!role"]
        }
  ...
  describe("User", () => {
    ...
    it("Should can't delete user", async () => {
      const response = await request.delete(`${rootUrl}/${grantList[0]._id}`);
      assert.equal(response.statusCode, 403);
    });
  });
